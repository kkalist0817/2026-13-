...
프론트엔드는 별도의 프레임워크 없이 **HTML, CSS, JavaScript**를 사용하여 구현했습니다.
프론트엔드 파일은 Spring Boot의 정적 리소스 경로인 `src/main/resources/static` 폴더에 위치하고, 백엔드 서버 실행 후 `http://localhost:8080/login.html` 주소를 통해 접속할 수 있습니다.

프론트엔드는 사용자가 서비스를 이용하는 전체 흐름인 **회원가입 → 로그인 → 로드맵 검색 → 상세 조회 → 구매 → 구매내역 확인 → 로드맵 생성** 과정을 화면 단위로 구성하였습니다. 또한 백엔드 API와 연동하여 Railway 클라우드 DB에 저장된 사용자, 로드맵, 구매내역 데이터를 화면에 반영하도록 구현하였습니다.
...
사용자는 ID와 비밀번호를 입력하여 로그인할 수 있으며, 입력값이 비어 있거나 로그인에 실패한 경우 오류 메시지가 표시됩니다.

로그인 기능은 프론트엔드 내부에서 ID/PW를 직접 비교하는 방식이 아니라 백엔드 로그인 API와 연동하여 처리되도록 구현하였습니다.
...
사용자는 ID, 비밀번호, 비밀번호 확인 값을 입력하여 회원가입을 진행할 수 있습니다.

회원가입 시 프론트엔드에서 입력값 누락 여부와 비밀번호 일치 여부를 먼저 확인하고, 조건을 만족하면 백엔드 회원가입 API로 요청을 전송합니다. 회원가입이 완료되면 로그인 화면으로 이동하도록 구현하였습니다.
...
메인 화면에서는 서비스명, 검색창, 로드맵 만들기 버튼, 내 구매내역을 확인할 수 있습니다.

신규 사용자는 구매내역이 비어 있는 상태로 시작하며 로드맵을 구매하면 메인 화면의 구매내역에 해당 로드맵이 반영됩니다.
...
검색 화면은 백엔드 로드맵 목록 조회 API와 검색 API를 사용하여 Railway DB에 저장된 로드맵 데이터를 화면에 렌더링합니다. 따라서 기본으로 등록된 로드맵뿐 아니라 사용자가 새로 생성한 로드맵도 검색 결과에 표시됩니다.
...
`loadmap-detail.html`은 사용자가 선택한 로드맵의 상세 정보를 확인하는 화면입니다.
상세 페이지는 URL의 로드맵 ID 값을 읽어 백엔드 상세 조회 API를 호출하고 해당 로드맵의 제목, 설명, 기간, 난이도, 시작 수준, 주차별 체크리스트를 화면에 표시합니다.

```text
GET /api/v1/roadmaps/{id}
```

상세 화면은 구매 전과 구매 후 화면을 구분하여 보여줍니다.

구매 전에는 로드맵의 기본 정보와 일부 체크리스트만 확인할 수 있고 전체 로드맵은 잠금 화면으로 표시됩니다. 구매 버튼을 누르면 백엔드 구매 API를 호출하여 구매 내역을 저장합니다.

```text
POST /api/v1/purchases
```

구매가 완료되면 전체 주차별 체크리스트가 표시되고, 사용자는 각 체크리스트 항목을 체크하면서 진행 상황을 관리할 수 있습니다.
...
`roadmap-create.html`은 사용자가 직접 새로운 로드맵을 생성하는 화면입니다.
사용자는 로드맵의 기본 정보와 주차별 체크리스트를 입력하여 자신만의 로드맵을 등록할 수 있습니다.

로드맵 생성 화면에서는 주차를 추가하거나 삭제할 수 있고, 각 주차 안에 체크리스트 항목을 여러 개 추가할 수 있습니다. 입력값 검사를 통과하면 백엔드 로드맵 생성 API로 데이터를 전송합니다.
...
프론트엔드의 주요 화면은 백엔드 API와 연동하여 동작합니다.
회원가입, 로그인, 로드맵 목록 조회, 로드맵 검색, 로드맵 상세 조회, 로드맵 생성, 로드맵 구매, 내 구매내역 조회 기능이 API 호출을 통해 처리됩니다.

#### 2. Railway DB 기반 데이터 공유

회원가입 정보, 로그인 계정, 로드맵 생성 데이터, 구매내역은 Railway 클라우드 DB를 기반으로 관리됩니다. 따라서 한 사용자가 생성한 로드맵은 다른 사용자도 검색을 통해 확인할 수 있으며, 계정별 구매내역은 로그인한 사용자 기준으로 구분됩니다.

#### 3. 구매 전/후 화면 분리

로드맵 상세 화면은 구매 여부에 따라 다른 화면을 제공합니다.
구매 전에는 로드맵 일부 정보와 잠금 화면을 보여주고, 구매 후에는 전체 주차별 체크리스트를 표시하여 사용자가 로드맵을 따라갈 수 있도록 하였습니다.

#### 4. 체크리스트 기반 진도 관리

구매한 로드맵은 주차별 체크리스트 형태로 제공됩니다.
사용자는 각 항목을 체크하면서 학습 또는 준비 과정을 단계별로 관리할 수 있습니다.

#### 5. 사용자 흐름 중심의 화면 구성

서비스의 핵심 사용 흐름은 다음과 같이 구성하였습니다.
